...
public abstract class HibernateDaoSupport<T> implements HibernateDaoHandler {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public List<T> getAll(final Class<T> entityClass, final String orderBy,
			final boolean isAsc) {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SinglePage<T> pagedQuery(String hql, int pageNo, int pageSize,
			Object... values) {
		Assert.hasText(hql);
	 ...
	 */
	@SuppressWarnings({ "unchecked" })
	public SinglePage<T> pagedQuery(Criteria criteria, int pageNo, int pageSize) {
	 ...
	 * @return
	 */
	public SinglePage<T> pagedQuery(Class<T> entityClass, int pageNo,
