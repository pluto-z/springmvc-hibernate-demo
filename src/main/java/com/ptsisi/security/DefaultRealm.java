...
public class DefaultRealm extends AuthorizingRealm {
  ...
  protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
    ...
    if (sessionPrincipal == null) { throw new ServiceException("session 变量对象不存在"); }
    User currentUser = sessionPrincipal.getUser();
    currentUser = userService.getUser(currentUser.getId());
    SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
    // 加载用户资源信息
    Set<Resource> resources = currentUser.getResources();
    // 添加用户拥有的permission
  ...
  protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
    ...
    if (user != null) {
      ...
      if (!user.isEnabled()) { throw new DisabledAccountException(user.getUsername() + " has been disabled"); }
