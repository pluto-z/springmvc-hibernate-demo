...
public class CaptchaAuthenticationFilter extends FormAuthenticationFilter {
	...
	@Override protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
		...
		if (number != null && number > this.allowIncorrectNumber) {
			...
			if (StringUtils.isEmpty(submitCaptcha) || !CaptchaProvider.getInstance()
					.validateResponseForID(httpServletRequest.getSession().getId(),
							submitCaptcha)) {
