...
(function($, undefined) {
	...
	$.validity = {
		...
		patterns : {
			...
			nonHtml : /^[^<>]*$/,
			notBlank : /[^\s]+/,
			chinaIdcard : /^\d{6}(18|19|20)?\d{2}(0[1-9]|1[012])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)?$/i,
			yearMonth : /^\d{1,4}-((0?\d)|(1[012]))$/
		},
	...
	$.fn
			.extend({
				...
				// The validity function is how validation can be bound to
				// forms. The user
				// may pass in a validation function or, as a shortcut, pass in
				// a string of
				...
				validity : function(arg) {
					...
					return this.each(function() {
						...
						if (this.tagName.toLowerCase() == "form") {
							...
							// If the user entered a string to select the inputs
							// to
							...
							// If the user entered a validation function then
							// just call
				...
				match : function(rule, msg) {
					...
					// If the
					// discovered rule is a function use it as such. Otherwise,
					// assume
					// it's a RegExp.
					$.isFunction(rule) ?
					...
					function(obj) {
						// Fix for regexes where the global flag is set. Make
						// sure
				...
				range : function(min, max, msg) {
					...
					orEmpty(function(obj) {
						...
						return (n.greaterThanOrEqualTo(new Big(min)) && n
								.lessThanOrEqualTo(new Big(max)));
					}) :
				...
				greaterThanOrEqualTo : function(min, msg) {
					...
					orEmpty(function(obj) {
						return new Big(obj.value).greaterThanOrEqualTo(new Big(
								min));
					}) :
				...
				alphabet : function(alpha, msg) {
					...
					return validate(this, function(obj) {
						...
						// For each character in the string, ensure that it's in
						// the
						// alphabet definition:
					...
					return validate(
							...
							orEmpty(function(obj) {
								return (obj.value.match(charClass) || []).length >= min;
									...
									|| format($.validity.messages.minCharClass,
											{
												min : min,
												charClass : charClass
											}));
					...
					return validate(
							...
							function(obj) {
								return (obj.value.match(charClass) || []).length <= max;
									...
									|| format($.validity.messages.maxCharClass,
											{
												max : max,
												charClass : charClass
											}));
				...
				// Validate that the input does not contain potentially
				// dangerous strings.
				...
				equal : function(arg0, arg1) {
					...
					var
					// If a reduced set is attached, use it. Also, remove
					// unsupported
					// elements.
					...
					if ($reduction.length) {
						...
						// If any value is not equal to the first value, then
						// they aren't
					...
					if ($reduction.length) {
						...
						for (var i1 = 0; i1 < map.length; ++i1) {
							...
							if (map[i1].length) {
								...
								for (var i2 = 0; i2 < subMap.length; ++i2) {
									// If we've already seen the transformed
									// value:
									if (subMap[i2] == map[i1]) {
										valid = false;
						...
						if (!valid) {
							...
							for (var i = 0, repeatedLength = repeatedVal.length; i < repeatedLength; ++i) {
								...
								// raise the error - aggregate will use last
								// repeated value
								raiseAggregateError($reduction
										.filter("[value=\'" + repeatedVal[i]
												+ "\']"), msg);
							}
				...
				// Validate that the numeric sum of all values is equal to a
				// given value.
				...
				// Validates an inclusive upper-bound on the numeric sum of the
				// values of
				...
				sumMax : function(max, msg) {
					...
					if ($reduction.length && max < numericSum($reduction)) {
						raiseAggregateError($reduction, msg
								|| format($.validity.messages.sumMax, {
									max : max
								}));
				...
				// Validates an inclusive lower-bound on the numeric sum of the
				// values of
				// all matched elements.
				sumMin : function(min, msg) {
					...
					if ($reduction.length && min > numericSum($reduction)) {
						raiseAggregateError($reduction, msg
								|| format($.validity.messages.sumMin, {
									min : min
								}));
				...
				radioChecked : function(val, msg) {
					...
					if ($reduction.is(":radio")
							&& $reduction.find(":checked").val() != val) {
						raiseAggregateError($reduction, msg
								|| $.validity.messages.radioChecked);
					}
				},
				...
				radioNotChecked : function(val, msg) {
					...
					if ($reduction.is(":radio")
							&& $reduction.filter(":checked").val() == val) {
						raiseAggregateError($reduction, msg
								|| $.validity.messages.radioChecked);
					}
				},
				...
				// If expression is a function, it will be called on each
				// matched element.
				...
				assert : function(expression, msg) {
					...
					// If a reduced set is attached, use it. Do not reduce to
					// supported
					// elements.
					var $reduction = this.reduction || this;
					...
					if ($reduction.length) {
						...
						// In the case that 'expression' is a function, use it
						// as a
						// regimen on each matched element individually:
						if ($.isFunction(expression)) {
							return validate(this, expression, msg
									|| $.validity.messages.generic);
						}
	...
	function capitalize(sz) {
		return sz.substring ? sz.substring(0, 1).toUpperCase()
				+ sz.substring(1, sz.length) : sz;
	}
...
(function($) {
	$.validity.outputs.tooltip = {
		...
		raise : function($obj, msg) {
			$obj.addClass("validity-tooltip-obj");
			var pos = $obj.offset();
			pos.left += $obj.width() + 45;
			pos.top += 8;

			var tooltip = $(
					"<div class=\"validity-tooltip\">" + msg
			...
			$obj.focus(function() {
				...
				$obj.removeClass("validity-tooltip-obj");
			});
...
(function($) {
	...
	$.validity.outputs.label = {
		...
		start : function() {
			...
			if (!results.valid && $.validity.settings.scrollTo) {
				location.hash = $(
						"." + $.validity.outputs.label.cssClass + ":eq(0)")
						.attr('for');
			}
		},

		raise : function($obj, msg) {
			var labelSelector = "." + $.validity.outputs.label.cssClass
			...
			// Otherwize create a new one and stick it after the input:
				...
				var errorLabel = $("<label/>").attr("for", getIdentifier($obj))
						.addClass($.validity.outputs.label.cssClass).text(msg)
		...
		raise : function($obj, msg) {
			if ($obj.length) {
				...
				$("<div/>")
						...
						.text(msg)
						.click(
								$.validity.settings.modalErrorsClickable ? function() {
									$(this).remove();
								}
...
(function($) {
	...
	$.validity.outputs.summary = {
		...
		start : function() {
			...
			if (buffer.length) {
				...
				for (var i = 0; i < buffer.length; ++i) {
					$(wrapper).text(buffer[i]).appendTo(
							this.options.container + " ul");
				}
		...
		container : function() {
			document.write("<div class=\"" + this.options.container + "\">"
